...
import { weatherApiService, geoApiService } from '@/services/api.service';
...
const fetchWeatherData = () => {
  ...
  const getCurrentWeatherData = async (lat, lon) => {
    return await weatherApiService.get(`weather?lat=${lat}&lon=${lon}&units=metric&appid=${process.env.VUE_APP_API_KEY}`);
  };
